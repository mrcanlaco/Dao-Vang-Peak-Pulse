...
export const Header: React.FC<HeaderProps> = ({
  ...
  const getModelDescription = (description?: string) => description
  ...
  const watchlistButton = (
    <button
      ...
      title="Chọn danh sách coin để quét"
    >
      <Target className="h-3.5 w-3.5 shrink-0" />
      <span className="truncate whitespace-nowrap">
        Danh sách quét: <strong className="font-mono uppercase">{scanLabel}</strong>
      </span>
    </button>
  );
      ...
      <div className="mx-auto flex max-w-[1700px] flex-col gap-2.5">
        <div className="flex flex-col gap-2.5 lg:flex-row lg:items-center lg:gap-4">
          ...
          <div className="flex min-w-0 items-center justify-between gap-3 lg:w-[22rem] lg:shrink-0 lg:justify-start">
            ...
            >
              ...
              <div className="min-w-0">
                <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-amber-400/80">Đảo Vàng AI</div>
            ...
            {availableModels.length > 0 && onSelectModel && (
              <div className="relative min-w-0">
                <button
                  ...
                  title={getModelDescription(selectedModel?.description) || 'Chọn mô hình AI'}
                >
                  <Cpu className="h-3 w-3 shrink-0" />
                  <span className="truncate">{getModelLabel(selectedModel?.label)}</span>
                  {isScannerActive && <span className="text-emerald-400" title="Đang dùng cho bộ quét 24/7">●</span>}
                ...
                {modelDropdownOpen && (
                  <>
                    <button
                      type="button"
                      aria-label="Đóng danh sách mô hình"
                    ...
                    <div className="absolute right-0 top-full z-50 mt-1 max-h-96 w-[min(20rem,calc(100vw-1.5rem))] overflow-y-auto rounded-lg border border-slate-700 bg-slate-900 p-2 shadow-xl lg:left-0 lg:right-auto">
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex min-w-0 items-center gap-1.5">
                              ...
                              {model.frozen_model_id === scannerModelId && scannerModelId && (
                                <span className="shrink-0 font-mono text-[9px] text-emerald-400">BỘ QUÉT</span>
                              )}
                            </div>
                          ...
                          {model.label_spec && (
                            <div className="mt-1 flex gap-2 font-mono text-[9px] text-slate-500">
                              <span>Mục tiêu: {model.label_spec.target_pct}</span>
                              <span>MAE: {model.label_spec.mae_pct}</span>
                              <span>Khung: {model.label_spec.horizon_h}</span>
                            </div>
                          )}
                        </button>
                      ))}
                      <div className="mt-1 border-t border-slate-800 px-2 py-1.5 text-[9px] text-slate-500">
                        {isScannerActive
                          ? '✅ Mô hình này đang được dùng cho bộ quét 24/7.'
                          : scannerModelId
                            ? 'ℹ️ Bộ quét đang dùng mô hình khác.'
                            : 'ℹ️ Bộ quét chưa cài mô hình đóng băng.'}
                      </div>
                    </div>
                  </>
                )}
          ...
          <div className="header-status-rail flex min-w-0 flex-1 items-center gap-2 overflow-x-auto pb-0.5 lg:border-l lg:border-slate-800 lg:pl-4">
            ...
            <div className="inline-flex min-w-max items-center gap-1.5 rounded-full border border-red-800/60 bg-red-950/70 px-2.5 py-1 font-mono text-[10px] font-bold text-red-400">
              ...
              <span>{status?.active_signals_count || 6} cảnh báo</span>
            </div>
            {autoTelegramEnabled && (
          ...
          <div className="flex shrink-0 items-center justify-end gap-1.5">
            ...
            <button
              ...
              title="Mở danh sách theo dõi tiến trình"
            >
            ...
            <button
              ...
              title={isActionDrawerOpen ? 'Ẩn tự động hóa thông minh' : 'Hiện tự động hóa thông minh'}
            >
              {isActionDrawerOpen ? <PanelRightClose className="h-3.5 w-3.5" /> : <PanelRight className="h-3.5 w-3.5" />}
              <span className="hidden text-[10px] font-bold sm:inline">Tự động</span>
            </button>
            <button
              type="button"
              onClick={onRefresh}
            ...
            <button
              ...
              title="Trợ giúp & Từ điển"
            >
              <HelpCircle className="h-3.5 w-3.5" />
              <span className="hidden text-xs font-medium sm:inline">Trợ giúp</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 border-t border-slate-800/70 pt-2 lg:flex-row lg:items-center lg:justify-between">
          <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_8.5rem] gap-2 sm:flex sm:flex-wrap sm:items-center lg:flex-nowrap">
            ...
            <label className="relative col-start-1 row-start-2 min-w-0 sm:w-56 lg:w-64" aria-label="Tìm coin">
              <Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" />
              <input
                type="text"
                placeholder="Tìm coin (BTC, SOL...)"
                value={searchTerm}
            ...
            <select
              ...
              aria-label="Lọc mức rủi ro"
            >
              <option value="ALL">Mọi mức rủi ro</option>
              <option value="CRITICAL">🔴 Cực Cao</option>
            ...
            <div className="hidden h-9 items-center gap-2 rounded-lg border border-slate-800 bg-slate-950 px-2.5 text-xs sm:flex">
              ...
              <span className="text-[11px] text-slate-400">Ngưỡng</span>
