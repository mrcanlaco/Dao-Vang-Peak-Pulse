...
export const TrackingWatchlist: React.FC<TrackingWatchlistProps> = ({
  ...
  const statusLabel: Record<TrackingStatus, string> = {
    WATCHING: isEn ? 'WATCHING' : 'ĐANG THEO DÕI',
    IN_POSITION: isEn ? 'IN POSITION' : 'ĐANG VÀO LỆNH',
    CLOSED: isEn ? 'CLOSED' : 'ĐÃ ĐÓNG',
  };

  const signalStatusLabel: Record<string, string> = {
    ACTIVE: isEn ? 'Radar Active' : 'Radar còn hiệu lực',
    HIT: isEn ? 'Target Hit (-8%)' : 'Radar đã trúng mục tiêu',
    EXPIRED: isEn ? 'Radar Expired' : 'Radar hết hạn',
    NO_SIGNAL: isEn ? 'Manual Track' : 'Theo dõi thủ công',
  };
    ...
    <div className="flex-1 min-h-0 overflow-y-auto space-y-3 pr-1">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-slate-100">
            ...
            {isEn ? 'Signal & PnL Performance Tracking' : 'Theo dõi tiến trình & Hiệu quả PnL'}
          </h2>
          <p className="mt-1 text-[11px] text-slate-500">
            {isEn ? 'Independent performance tracking pool from 24/7 scanner alerts.' : 'Tách khỏi danh sách coin mà scanner dùng để quét.'}
          </p>
        </div>
        <button
          type="button"
        ...
        >
          ...
          {isEn ? 'Refresh Prices' : 'Cập nhật giá'}
        </button>
      </div>

      <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
        <div className="rounded-xl border border-amber-800/60 bg-amber-950/30 p-3">
          <div className="text-[10px] uppercase text-slate-500">{isEn ? 'Total Tracked' : 'Đang theo dõi'}</div>
        ...
        <div className="rounded-xl border border-sky-800/60 bg-sky-950/30 p-3">
          <div className="text-[10px] uppercase text-slate-500">{isEn ? 'Radar Active' : 'Radar còn hiệu lực'}</div>
        ...
        <div className="rounded-xl border border-purple-800/60 bg-purple-950/30 p-3">
          <div className="text-[10px] uppercase text-slate-500">{isEn ? 'In Position' : 'Đang vào lệnh'}</div>
        ...
        <div className="rounded-xl border border-red-800/60 bg-red-950/30 p-3">
          <div className="text-[10px] uppercase text-slate-500">{isEn ? 'Attention Needed' : 'Cần chú ý'}</div>
      ...
      <div className="flex items-center gap-1 overflow-x-auto rounded-xl border border-slate-800 bg-slate-950 p-1 [&::-webkit-scrollbar]:hidden">
        {([
          ...
          ['WATCHING', isEn ? 'Watching Only' : 'Chỉ theo dõi'],
          ['IN_POSITION', isEn ? 'In Position' : 'Đang vào lệnh'],
          ['CLOSED', isEn ? 'Closed' : 'Đã đóng'],
      ...
      {isLoading && items.length === 0 ? (
        <div className="rounded-xl border border-slate-800 bg-slate-950 p-10 text-center text-xs text-slate-500">
          ...
          {isEn ? 'Loading tracking list...' : 'Đang tải danh sách theo dõi...'}
        </div>
        ...
        <div className="rounded-xl border border-dashed border-slate-800 bg-slate-950/60 p-10 text-center">
          ...
          <div className="text-xs font-semibold text-slate-400">
            {isEn ? 'No coins in this group' : 'Chưa có coin trong nhóm này'}
          </div>
          <div className="mt-1 text-[11px] text-slate-600">
            {isEn ? 'Click "Track" on any Radar alert to start monitoring performance.' : 'Bấm "Theo dõi" trên một cảnh báo Radar để bắt đầu.'}
          </div>
        </div>
      ) : (
              ...
              <article key={item.id} className="rounded-xl border border-slate-800 bg-slate-950/80 p-3 shadow-lg shadow-black/10">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
                  ...
                  <div className="grid flex-1 grid-cols-2 gap-2 sm:grid-cols-4">
                    ...
                    <div className="rounded-lg border border-slate-800 bg-slate-900/70 p-2">
                      <div className="text-[9px] uppercase text-slate-500">{isEn ? 'Current Price' : 'Giá hiện tại'}</div>
                      ...
                      <div className={`text-[10px] ${positiveSignalChange ? 'text-emerald-400' : 'text-red-400'}`}>
                        {formatPercent(item.signal_change_pct, true)} {isEn ? 'from Radar' : 'từ Radar'}
                      </div>
                    </div>
                    <div className="rounded-lg border border-slate-800 bg-slate-900/70 p-2">
                      <div className="text-[9px] uppercase text-slate-500">{isEn ? 'Target Progress' : 'Tiến trình mục tiêu'}</div>
                    ...
                    <div className="rounded-lg border border-slate-800 bg-slate-900/70 p-2">
                      <div className="text-[9px] uppercase text-slate-500">{isEn ? 'Position / PnL' : 'Vị thế / PnL'}</div>
                      ...
                      ) : <div className="mt-0.5 text-xs text-slate-500">{isEn ? 'No position entered' : 'Chưa nhập lệnh'}</div>}
                    </div>
                  </div>
                  ...
                  <div className="flex shrink-0 items-center gap-1.5 lg:w-44 lg:justify-end">
                    {item.status !== 'CLOSED' && (
                      <button type="button" onClick={() => openEditor(item)} disabled={isUpdating} className="inline-flex h-8 items-center gap-1 rounded-lg border border-slate-700 bg-slate-900 px-2.5 text-[10px] font-semibold text-slate-300 transition hover:border-amber-500/60 hover:text-amber-300 disabled:opacity-50">
                        ...
                        {item.status === 'IN_POSITION' ? (isEn ? 'Edit Position' : 'Sửa lệnh') : (isEn ? 'Enter Position' : 'Đã vào lệnh')}
                      </button>
                    )}
                    {item.status === 'IN_POSITION' && (
                      <button type="button" onClick={() => void closeTracking(item)} disabled={isUpdating} className="inline-flex h-8 items-center gap-1 rounded-lg border border-emerald-800/70 bg-emerald-950/40 px-2.5 text-[10px] font-semibold text-emerald-300 transition hover:bg-emerald-950 disabled:opacity-50">
                        <CheckCircle2 className="h-3 w-3" /> {isEn ? 'Close Position' : 'Đóng lệnh'}
                      </button>
                    )}
                    {item.status === 'CLOSED' && (
                      <button type="button" onClick={() => void onRemoveItem(item.id)} disabled={isUpdating} className="inline-flex h-8 items-center gap-1 rounded-lg border border-red-900/70 bg-red-950/40 px-2.5 text-[10px] font-semibold text-red-300 transition hover:bg-red-950 disabled:opacity-50">
                        <Archive className="h-3 w-3" /> {isEn ? 'Delete' : 'Xóa'}
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-slate-800/80 pt-2 text-[10px] text-slate-500">
                  <span className="inline-flex items-center gap-1">
                    ...
                    {item.validity_hours_left == null 
                      ? (isEn ? 'No expiration' : 'Không có thời hạn') 
                      : item.validity_hours_left > 0 
                        ? (isEn ? `${item.validity_hours_left.toFixed(1)}h left` : `Còn ${item.validity_hours_left.toFixed(1)} giờ`) 
                        : (isEn ? 'Expired' : 'Đã hết hạn')}
                  </span>
                  <span>{isEn ? 'Target' : 'Mục tiêu'} {formatPrice(item.source_target_price)}</span>
                  {item.status === 'IN_POSITION' && <span>Entry {formatPrice(item.entry_price)} · {item.position_side}</span>}
                  {item.last_market_update && <span>{isEn ? 'Updated' : 'Cập nhật'} {formatSystemTime(item.last_market_update)}</span>}
                </div>
                  ...
                  <form onSubmit={(event) => void submitPosition(event, item)} className="mt-3 rounded-xl border border-amber-800/60 bg-amber-950/20 p-3">
                    <div className="mb-2 flex items-center justify-between">
                      <div className="text-xs font-bold text-amber-300">
                        {isEn ? 'Binance Position Tracker (Manual Entry)' : 'Thông tin lệnh Binance (nhập thủ công)'}
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                      <label className="text-[10px] text-slate-400">{isEn ? 'Side' : 'Hướng'}<select value={form.position_side} onChange={event => setForm(prev => ({ ...prev, position_side: event.target.value as 'LONG' | 'SHORT' }))} className="mt-1 h-8 w-full rounded-lg border border-slate-700 bg-slate-950 px-2 text-xs text-slate-200"><option value="SHORT">SHORT</option><option value="LONG">LONG</option></select></label>
                      <label className="text-[10px] text-slate-400">{isEn ? 'Entry Price' : 'Giá vào'}<input required type="number" step="any" min="0" value={form.entry_price} onChange={event => setForm(prev => ({ ...prev, entry_price: event.target.value }))} className="mt-1 h-8 w-full rounded-lg border border-slate-700 bg-slate-950 px-2 text-xs text-slate-200" /></label>
                      <label className="text-[10px] text-slate-400">{isEn ? 'Quantity' : 'Số lượng coin'}<input type="number" step="any" min="0" value={form.quantity} onChange={event => setForm(prev => ({ ...prev, quantity: event.target.value }))} className="mt-1 h-8 w-full rounded-lg border border-slate-700 bg-slate-950 px-2 text-xs text-slate-200" /></label>
                      <label className="text-[10px] text-slate-400">{isEn ? 'Notional USDT' : 'Notional USDT'}<input type="number" step="any" min="0" value={form.notional} onChange={event => setForm(prev => ({ ...prev, notional: event.target.value }))} className="mt-1 h-8 w-full rounded-lg border border-slate-700 bg-slate-950 px-2 text-xs text-slate-200" /></label>
                      <label className="text-[10px] text-slate-400">{isEn ? 'Leverage' : 'Đòn bẩy'}<input type="number" step="any" min="0" value={form.leverage} onChange={event => setForm(prev => ({ ...prev, leverage: event.target.value }))} className="mt-1 h-8 w-full rounded-lg border border-slate-700 bg-slate-950 px-2 text-xs text-slate-200" /></label>
                      ...
                      <label className="text-[10px] text-slate-400 sm:col-span-1">{isEn ? 'Notes' : 'Ghi chú'}<input value={form.notes} onChange={event => setForm(prev => ({ ...prev, notes: event.target.value }))} className="mt-1 h-8 w-full rounded-lg border border-slate-700 bg-slate-950 px-2 text-xs text-slate-200" /></label>
                    </div>
                    <div className="mt-2 flex items-center justify-between gap-2">
                      <span className="text-[10px] text-slate-500">
                        {isEn ? 'PnL excludes exchange fees, funding and liquidation limits.' : 'PnL chưa bao gồm phí, funding và giá thanh lý.'}
                      </span>
                      <button type="submit" disabled={isUpdating} className="inline-flex h-8 items-center gap-1 rounded-lg bg-amber-500 px-3 text-[10px] font-bold text-slate-950 hover:bg-amber-400 disabled:opacity-50">
                        <Activity className="h-3 w-3" /> {isEn ? 'Save Position' : 'Lưu vị thế'}
                      </button>
                    </div>
                  </form>
                )}
