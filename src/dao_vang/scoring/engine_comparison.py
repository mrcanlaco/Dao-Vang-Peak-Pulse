...
snapshots to compare the two engines under one documented evaluation rule.
"""
...
class EnginePerformanceSummary:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "mean_lead_time_min": (
                round(self.mean_lead_time_min, 1)
                if self.mean_lead_time_min is not None
                else None
            ),
...
def evaluate_scoring_engines_comparison(
    ...
    try:
        ...
        return _unavailable_engine_comparison("historical_query_failed")

    if df_features.empty:
        return _unavailable_engine_comparison("no_historical_candidates")
    ...
    for _, row in df_features.iterrows():
        ...
        try:
            future_klines = conn.execute(
                """
                SELECT close_time, high, low, close FROM kline
        ...
        max_high = max(float(k[1]) for k in future_klines)
        min_low = min(float(k[2]) for k in future_klines)
            ...
            if (entry_price - k_low) / entry_price >= 0.04:
                if not hit_tp1:
                    try:
                        lead_time_min = max(
                            0.0,
                            float((k_time - feature_time).total_seconds()) / 60.0,
                        )
    ...
    if summary_v1.total_signals == 0 and summary_v2.total_signals == 0:
        return _unavailable_engine_comparison("no_qualifying_signals")

    v2_wins = summary_v2.precision_score >= summary_v1.precision_score
    return {
        "status": "success",
        "source": "duckdb_point_in_time_evaluation",
        "sample_count": len(df_features),
        "evaluated_at": datetime.now(timezone.utc).isoformat(),
        "champion_engine": summary_v2.to_dict() if v2_wins else summary_v1.to_dict(),
        "comparison": {
            "v1": summary_v1.to_dict(),
            "v2": summary_v2.to_dict(),
        },
        "verdict": {
            "winner": "V2 (2-Tier Climax Engine)" if v2_wins else "V1 (Heuristic Composite)",
            ...
            "explanation": (
                "Kết quả được tính từ các ứng viên và nến tương lai có sẵn trong DuckDB; "
                "hãy đọc cùng cỡ mẫu, thời điểm đánh giá và quy tắc TP/SL."
            ),
        },
    }
...
def _calculate_summary(signals: list[dict[str, Any]], name: str, version: str) -> EnginePerformanceSummary:
    ...
    sl_rate = sl / total
    observed_leads = [
        float(signal["lead_time_min"])
        for signal in signals
        if signal.get("lead_time_min") is not None
    ]
    mean_lead_time_min = (
        sum(observed_leads) / len(observed_leads)
        if observed_leads
        else None
    )
    ...
    return EnginePerformanceSummary(
        ...
        avg_mfe=avg_mfe,
        avg_risk_reward=rr,
        mean_lead_time_min=mean_lead_time_min,
        precision_score=precision_score,
    )


def _unavailable_engine_comparison(reason: str) -> dict[str, Any]:
    """Return an explicit no-data contract; never synthesize benchmark metrics."""
    return {
        "status": "unavailable",
        "reason": reason,
        "source": "none",
