...
def test_oi_collector_success(tmp_path: Path) -> None:
    ...
    collector = OpenInterestCollector(mock_client, settings)
    ...
    target_file = (
        tmp_path / "raw" / "open_interest" / f"date={date_str}" / "test-run-id.jsonl"
    )
...
def test_oi_collector_api_error(tmp_path: Path) -> None:
    settings = AppSettings()
    settings.paths.data_dir = tmp_path
