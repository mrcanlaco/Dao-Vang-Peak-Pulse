"""
Mid-Cap and Top-Cap Coins 365-Day Backtest Pipeline
Outputs results to artifacts/agent2_backtest_mid_top.json
"""
...
OUTPUT_PATH = Path("artifacts/agent2_backtest_mid_top.json")
...
def run_mid_top_backtest():
    ...
    GROUP BY symbol
    HAVING avg_daily_vol > 500000000
    ORDER BY avg_daily_vol DESC
    """
    target_coins = [r[0] for r in conn.execute(midtop_query, [max_time]).fetchall()]
    logger.info(f"Identified {len(target_coins)} mid/top-cap coins")

    chunk_size = 30
    symbol_chunks = [target_coins[i:i + chunk_size] for i in range(0, len(target_coins), chunk_size)]
    ...
    logger.info(f"Total Mid/Top-Cap candidate signals dataset size: {len(df_cand):,} rows. Label positive rate: {df_cand['label'].mean():.4f}")
    ...
    results = {
        "metadata": {
            "dataset": "Mid/Top-Cap Perpetual Coins (> 500M USDT daily volume)",
            "period": "365 days",
