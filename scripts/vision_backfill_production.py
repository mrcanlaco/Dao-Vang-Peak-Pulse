...
def init_state_db(state_db: Path):
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS download_state (
            ...
            PRIMARY KEY (target_id, url)
        )
    """)
    conn.close()

def get_applied_urls(state_db: Path, target_id: str):
    if not state_db.exists(): return set()
    conn = duckdb.connect(str(state_db), read_only=True)
    rows = conn.execute("SELECT url FROM download_state WHERE target_id = ? AND status IN ('applied', 'unavailable')", [target_id]).fetchall()
    conn.close()
    return set(r[0] for r in rows)

def mark_state(state_db: Path, target_id: str, url: str, status: str, checksum: str = ""):
    conn = duckdb.connect(str(state_db))
    conn.execute("""
        INSERT INTO download_state (target_id, url, status, checksum) 
        VALUES (?, ?, ?, ?)
        ON CONFLICT (target_id, url) DO UPDATE SET status = EXCLUDED.status, checksum = EXCLUDED.checksum
    """, [target_id, url, status, checksum])
    conn.close()

def mark_applied(state_db: Path, target_id: str, urls: list[str]):
    conn = duckdb.connect(str(state_db))
...
async def fetch_vision_file(client, state_db: Path, target_id: str, url: str, out_csv: Path):
    if out_csv.exists():
        out_csv.unlink()
    try:
        ...
        mark_state(state_db, target_id, url, "validated", actual_hash)
        return "ok"
    except Exception:
        return "error"

async def fetch_funding_rest(client, state_db: Path, target_id: str, sym: str, start_ms: int, end_ms: int, out_csv: Path, url_id: str):
    if out_csv.exists():
        out_csv.unlink()
    try:
        ...
        if resp.status_code == 200:
            ...
            if data:
                ...
                mark_state(state_db, target_id, url_id, "validated", "rest_no_hash")
            return True
        return False
    except Exception:
        return False
...
def validate_parquet(path: Path, time_col: str):
    conn = duckdb.connect()
    try:
        res = conn.execute(f"""
            SELECT 
        ...
        rows, nulls, unique_keys = res
        if nulls > 0:
            raise ValueError(f"Validation failed: {nulls} NULL keys found!")
        if rows != unique_keys:
            raise ValueError(f"Validation failed: {rows - unique_keys} duplicate keys found!")
    finally:
        conn.close()

async def backfill_symbol(client, sym: str, onboard_time: pd.Timestamp, root_dir: Path, state_db: Path, test_mode: bool = False) -> bool:
    ...
    for ds in ["klines/5m", "metrics/5m", "funding"]:
        target_id = f"{sym}_{ds}"
        applied_urls = get_applied_urls(state_db, target_id)
        has_error = False
        ...
        while month_iter <= end_date:
            ...
            if ym == current_ym:
                if ds == "funding":
                    url_id = f"rest_funding_{sym}_{ym}"
                    if url_id not in applied_urls:
                        ...
                        fetch_tasks.append((fetch_funding_rest(client, state_db, target_id, sym, start_ms, end_ms, out_csv, url_id), out_csv, "funding_rest", url_id))
                else:
                    ...
                    for d in range(1, now_day):
                        ...
                        if url in applied_urls: continue
                        out_csv = tmp_dir / sym / ds / f"{ymd}.csv"
                        fetch_tasks.append((fetch_vision_file(client, state_db, target_id, url, out_csv), out_csv, ds, url))
            else:
        ...
        csv_paths = []
        if fetch_tasks:
            results = await asyncio.gather(*(t[0] for t in fetch_tasks))
            for res, (_, out_csv, ds_key, url) in zip(results, fetch_tasks):
                if res == "ok" or res == True:
                    csv_paths.append((out_csv, ds_key, url))
                elif res == "404":
                    mark_state(state_db, target_id, url, "unavailable")
                else:
                    print(f"  [{sym}] Download Error: {url}")
                    has_error = True
        ...
        if csv_paths:
            ...
            for q_key, paths_urls in grouped.items():
                ...
                try:
                    ...
                    validate_parquet(target, time_col)
                    mark_applied(state_db, target_id, urls)
...
async def main():
    ...
    parser.add_argument("--test", type=str, help="Symbol to test")
    parser.add_argument("--apply", action="store_true", help="Apply to D: drive")
    args = parser.parse_args()
    
    root_dir = Path("D:/Quant-trading/data_lake") if args.apply else Path("artifacts/smoke_v2")
    state_db = root_dir / "backfill_state_v2.duckdb"
    ...
    async with httpx.AsyncClient(limits=httpx.Limits(max_connections=50)) as client:
        if args.test:
            ...
            res = await backfill_symbol(client, args.test, onboard, root_dir, state_db, test_mode=True)
            ...
            for sym in universe:
                ...
                if not await backfill_symbol(client, sym, onboard, root_dir, state_db):
                    success = False
            if not success:
